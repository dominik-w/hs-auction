...
const main = async () => {
  ...
  const hCore = new Hypercore('./db/auction-client-bid');
  ...
  console.log('Bid by:', b4a.toString(hCore.key, 'hex'));
  ...
  const payload = {
    id: 'Pic#1',
    bid: 80,
    bidUserPK: b4a.toString(hCore.key, 'hex'),
  };

  const payloadRaw = Buffer.from(JSON.stringify(payload), 'utf-8');

  // Call makeBid.
  const responseRaw = await rpc.request(serverPK, 'makeBid', payloadRaw);
